...
export class HelloGoStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: HelloGoStackProps) {
    ...
    const { baseName, ephemeralNs, isEphemeral, ecrImage } = props;
    ...
    const lambdaName = `${lambdaBaseName}${ephemeralNs ? `-${ephemeralNs}` : ''}`;
    ...
    const lambdaFunction = new lambda.DockerImageFunction(
      ...
      'HelloGoLambda',
      {
        ...
        description: `hello-go API Lambda ${ephemeralNs ? ` (${ephemeralNs})` : ''}`,
    ...
    const httpApi = new apigatewayv2.HttpApi(this, 'HelloGoHttpApi', {
      apiName: lambdaName,
      description: `HTTP API for hello-go${ephemeralNs ? ` (${ephemeralNs})` : ''}`,
      corsPreflight: {
        allowOrigins: ['*'],
        allowMethods: [
